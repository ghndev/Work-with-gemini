...
export default function PuzzleApp({ isLoggedIn }: { isLoggedIn?: boolean }) {
  ...
  const { loading, error, setError, generateAI, generatePreset } =
    usePuzzleGenerator(async (newState, newPieces) => {
  ...
  const handleGenerate = () => {
    ...
    generateAI(prompt, aspectRatio, difficulty);
  };
  ...
  const handlePresetSelect = (
    ...
    // Force aspect ratio state to sync intuitively for users returning from a preset
    setAspectRatio('4:3');
    generatePreset(imageUrl, selectedDifficulty);
  };
