...
import { Loader2, Sparkles, Image as ImageIcon, Play, Settings2 } from 'lucide-react';
import { get, set, del } from 'idb-keyval';
import dynamic from 'next/dynamic';

// Dynamically import PuzzleBoard to avoid SSR issues with react-konva and canvas
...
export default function Page() {
  ...
  useEffect(() => {
    // Try to load saved puzzle on mount
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const newState = await createPuzzleState(imageUrl, cols, rows);
  ...
  return (
    <>
      ...
      <div className={`min-h-screen bg-zinc-950 text-zinc-100 flex-col items-center justify-center p-6 ${showPuzzle ? 'hidden' : 'flex'}`}>
